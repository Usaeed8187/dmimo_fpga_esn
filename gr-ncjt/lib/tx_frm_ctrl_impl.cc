...
tx_frm_ctrl::sptr
tx_frm_ctrl::make(int ntx, int framedatalen, const char *filename, double samplerate, int pktspersec,
                  double starttime, int padding, bool autostart, int delay, bool debug)
{
    return gnuradio::make_block_sptr<tx_frm_ctrl_impl>(ntx, framedatalen, filename, samplerate, pktspersec,
                                                       starttime, padding, autostart, delay, debug);
}

tx_frm_ctrl_impl::tx_frm_ctrl_impl(int ntx, int framedatalen, const char *filename, double samplerate, int pktspersec,
                                   double starttime, int padding, bool autostart, int delay, bool debug)
    : gr::block("tx_frm_ctrl",
      ...
      d_padding_length(padding), d_cur_frame_cnt(0), d_debug(debug)
{
    ...
    if ((d_beacon_data = read_beacon_data(filename, &d_beacon_len)) == nullptr)
...
void
tx_frm_ctrl_impl::forecast(int noutput_items, gr_vector_int &ninput_items_required)
{
    // input buffers can be empty
    for (int k = 0; k < d_ntx; k++)
...
int
tx_frm_ctrl_impl::general_work(int noutput_items, gr_vector_int &ninput_items,
                       ...
                       gr_vector_void_star &output_items)
{
    ...
    bool tx_data_ready = (min_input_items >= d_data_length);
    ...
    if (!tx_data_ready && d_txen && tx_time_secs <= d_txtime_start)
        return 0;
...
gr_complex *
tx_frm_ctrl_impl::read_beacon_data(const char *filename, int *beacon_len)
{
    FILE *d_fp;
    struct GR_STAT st;
