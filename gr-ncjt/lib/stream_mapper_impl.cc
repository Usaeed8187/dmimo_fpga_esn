...
stream_mapper::make(int rgmode,
                    ...
                    bool debug)
{
    return gnuradio::make_block_sptr<stream_mapper_impl>(
...
////////////////////////////////////////////////////////////////////
stream_mapper_impl::stream_mapper_impl(int rgmode,
                                       int nstrm,
                                       int phase1_modtype,
                                       int phase2_modtype,
                                       int phase3_modtype,
                                       ...
                                       bool deterministic_input,
                                       bool debug)
    : gr::tagged_stream_block("stream_mapper",
      ...
      d_seqno(0)
{
    ...
    if (d_nstrm < 1 || d_nstrm > 4)
        throw std::runtime_error("stream_mapper: invalid nstrm (1..4).");
    if (!d_valid_mod_types.count(d_phase1_modtype))
        throw std::runtime_error("stream_mapper: invalid modtype (2,4,6,8).");
    ...
    if (d_phase1_modtype < d_phase2_modtype)
        throw std::runtime_error("stream_mapper: phase1_modtype must be >= phase2_modtype.");
    if (d_phase3_modtype < d_phase2_modtype)
        throw std::runtime_error("stream_mapper: phase3_modtype must be >= phase2_modtype.");
    if (d_code_rate < 0 || d_code_rate > 7)
        throw std::runtime_error("stream_mapper: code_rate must be in [0,7].");
    ...
    else
    {
        throw std::runtime_error(
            "[stream_mapper_impl] ERROR: LDPC factories could not be created.");
    }
                        ...
                        if (pmt::is_number(msg))
                        {
                                ...
                                d_valid_mod_types.count(d_new_phase3_modtype))
                            {
                                    ...
                                    (d_new_phase2_modtype <= d_new_phase3_modtype))
                                {
                                    ...
                                    std::cout << "[stream_mapper_impl] Updated d_phase1_modtype to "
                                ...
                                {
                                    throw std::runtime_error(
                                        "stream_mapper: Phase 2 modtype must be less than or equal to both phase 1 and phase 3 modtypes. "
                            ...
                            {
                                throw std::runtime_error("stream_mapper: Invalid received modtype message.");
                            }
                        }
                        else
                        {
    ...
    set_msg_handler(pmt::intern("s_code_rate"), [this](pmt::pmt_t msg)
    {
        if (pmt::is_number(msg))
        {
            ...
            {
                d_code_rate = new_cr;
                std::cout << "[stream_mapper_impl] Updated d_code_rate to "
...
// forecast()
////////////////////////////////////////////////////////////////////
/*void stream_mapper_impl::forecast(int noutput_items,
...
int
stream_mapper_impl::calculate_output_stream_length(const gr_vector_int &ninput_items)
{
...
////////////////////////////////////////////////////////////////////
int
stream_mapper_impl::work(int noutput_items,
                         ...
                         gr_vector_void_star &output_items)
{
    ...
    if (!d_deterministic_input)
    {
        ...
        NCJT_LOG(d_debug,
                 "Pulled " << inp_bits_avail << " bits from input; d_bit_buffer.size()=" << d_bit_buffer.size());
    }
    ...
    //--------------------------------------------------------------------
    int frame_data_syms =
        d_nstrm * (d_n_ofdm_syms * d_sd_num - 64); // TODO: What if RG dimensions differ across phases?
    ...
    for (int k = 0; k < num_periods; k++)
    {
        ...
        for (int s = 0; s < d_nstrm; s++)
        {
            ...
            {
                case 2:sym = (*d_constellation_qpsk)[val];
                    break;
                case 4:sym = (*d_constellation_16qam)[val];
                    break;
                case 6:sym = (*d_constellation_64qam)[val];
                    break;
                case 8:sym = (*d_constellation_256qam)[val];
                    break;
                default:throw std::runtime_error("Invalid modulation type");
            }
