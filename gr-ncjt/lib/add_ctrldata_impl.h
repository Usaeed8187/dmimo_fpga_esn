...
#ifndef INCLUDED_NCJT_ADD_CTRLDATA_IMPL_H
#define INCLUDED_NCJT_ADD_CTRLDATA_IMPL_H
...
class add_ctrldata_impl : public add_ctrldata
{
private:
    ...
    bool d_add_txseq; // add sequence numbers at TxUE
    bool d_add_rxseq; // add sequence numbers at RxUE
    bool d_add_llr; // add LLR data at RxUE
    ...
    uint8_t  *d_llr_data;  // 6-bit LLR values
...
protected:
    ...
    void encode_ctrl_data(int chan, uint8_t *outbuf);

    void decode_ctrl_data(int chan, const uint8_t *data, int datalen);

    void
    process_llr_message(const pmt::pmt_t &msg);

public:
    add_ctrldata_impl(int nstrm, int ctrl_datalen, bool add_txseq, bool add_rxseq,
                      bool add_llr, bool debug);
    ~add_ctrldata_impl();
...
#endif /* INCLUDED_NCJT_ADD_CTRLDATA_IMPL_H */
