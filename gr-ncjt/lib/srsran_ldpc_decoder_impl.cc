...
srsran_ldpc_decoder::make(std::string mod_order,
                          int message_length,
                          int codeword_length,
                          bool is_output_byte,
                          bool debug)
...
srsran_ldpc_decoder_impl::srsran_ldpc_decoder_impl(std::string mod_order,
                                                   int message_length,
                                                   int codeword_length,
                                                   bool is_output_byte,
                                                   bool debug)
    : gr::tagged_stream_block("srsran_ldpc_decoder",
      ...
      d_is_output_byte(is_output_byte),
      d_debug(debug)
{
    ...
    d_output_stream_len = d_is_output_byte ? d_message_length / 8
                                           : d_message_length; // assume message_length is multiple of 8 if is_output_byte
...
{
    int num_ldpc_blocks = ninput_items[0] / d_message_length;
    return num_ldpc_blocks * d_output_stream_len;
}
...
{
    if (ninput_items[0] % d_codeword_length != 0)
    {
        std::cerr << "Got wrong codeword length" << ninput_items[0] <<
                  ", expect " << d_codeword_length << std::endl;
        return 0;
    }
    int num_ldpc_blocks = ninput_items[0] / d_codeword_length;
    if (noutput_items < num_ldpc_blocks * d_output_stream_len)
    ...
    {
        int input_offset = blk * d_codeword_length;
        int output_offset = blk * d_output_stream_len;

        for (int i = 0; i < d_codeword_length; ++i)
        {
            d_llrs[i] = log_likelihood_ratio::quantize(in[i + input_offset], 15); // Convert to LLRs
        }
        ...
        {
            std::copy(d_decoded.get_buffer().begin(),
                      d_decoded.get_buffer().begin() + d_output_stream_len,
                      out + output_offset);
        }
        else                 // bit output case
        {
            srsvec::bit_unpack(d_decoded_bits_unpacked, d_decoded);
            std::copy(d_decoded_bits_unpacked.begin(),
                      d_decoded_bits_unpacked.end() - d_cfg.cb_specific.nof_filler_bits,
                      out + output_offset);
        }
    }
    ...
    add_item_tag(0, nitems_written(0),
                 ...
                 pmt::from_long(num_ldpc_blocks * d_output_stream_len),
    ...
    dout << "Decoding Speed:" << (num_ldpc_blocks * d_message_length) / float(de_d.count()) * 0.1192
