      ...
      d_num_symbols(ndatasymbols), d_debug(debug)
{
    ...
    switch (modtype){
        case 2:
            QAM_CONST = CONST_QPSK;
            break;
        case 4:
            QAM_CONST = CONST_16QAM;
            break;
        case 6:
            QAM_CONST = CONST_64QAM;
            break;
        case 8:
            QAM_CONST = CONST_256QAM;
            break;
        default:
...
sic_detect_impl::update_mmse_sic_coef(float nvar)
{
    ...
    for (int k = 0; k < d_scnum; k++)
    {
        ...
        Eigen::Map<const CMatrixX> ch(&d_chan_est_buf[d_nrx * d_nss * k], d_nrx, d_nss);
        CMatrixX H(d_nrx, d_nss);
        H = ch; // copy data from buffer

        std::vector<int> stream_indices(d_nss); // original indices for remaining stream
        for (int n = 0; n < d_nss; n++) stream_indices[n] = n;
