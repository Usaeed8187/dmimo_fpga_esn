...
namespace gr::ncjt {
    ...
    tx_framing::make(int ntx,
        ...
        return gnuradio::make_block_sptr<tx_framing_impl>(
                ntx, ndatasyms, filename, fs, pktspersec, starttime, padding, csirs, debug);
    }

    tx_framing_impl::tx_framing_impl(int ntx,
                                     int ndatasyms,
                                     const char *filename,
                                     double fs,
                                     int pktspersec,
            ...
            : gr::tagged_stream_block("tx_framing",
                                      ...
                                      gr::io_signature::make(ntx, ntx, sizeof(gr_complex)),
                                      "packet_len"),
              d_padding_length(padding), d_frame_cnt(0), d_csirs(csirs), d_debug(debug) {
        if (ntx < 1 || ntx > 8)
        ...
        if (starttime < 0.0 || starttime > 10.0)
    ...
    tx_framing_impl::process_rxtime_message(const pmt::pmt_t &msg) {
        ...
        if (abs(next_txtime - schedule_txtime) < 2e-3 * d_repeat_interval)
            return;
    ...
    tx_framing_impl::work(int noutput_items, gr_vector_int &ninput_items,
        ...
        } else  // send normal packets if transmitter is enabled
        {
            ...
            if (d_csirs) {
                ...
                double time_fracs = d_time_fracs + 0.25 * d_repeat_interval;
                double time_intpart; // normalize
                time_fracs = std::modf(time_fracs, &time_intpart);
                uint64_t time_secs = d_time_secs + uint64_t(time_intpart);
                ...
                for (int ch = 0; ch < d_ntx; ch++) {
                    ...
                    // Add tags for burst transmission
                    add_item_tag(ch,
                                 nitems_written(ch) + d_frame_length,
                                 pmt::string_to_symbol("tx_pkt_len"),
                                 pmt::from_long(2 * d_padding_length + d_beacon_len),
                                 _id);

                    const pmt::pmt_t tx_time =
                            pmt::make_tuple(pmt::from_uint64(time_secs), pmt::from_double(time_fracs));
                    add_item_tag(ch,
                                 nitems_written(ch) + d_frame_length,
                                 pmt::string_to_symbol("tx_time"),
                                 tx_time,
                    ...
                    // zero-padding and copy beacon symbols
                    if (ch == cur_ti) {
                        memset((void *) &out[d_frame_length], 0, sizeof(gr_complex) * d_padding_length);
                        memcpy(&out[d_frame_length + d_padding_length],
                               &d_beacon_data[160],
                               sizeof(gr_complex) * d_beacon_len);
                        // zero-padding after frame
                        memset((void *) &out[d_frame_length + d_beacon_len + d_padding_length],
                               0,
                               sizeof(gr_complex) * d_padding_length);
                    } else {
                        memset((void *) &out[d_frame_length],
                               0,
                               sizeof(gr_complex) * (2 * d_padding_length + d_beacon_len));
                    }
