  ...
  namespace ncjt {
    ...
    mapper_muxer::sptr mapper_muxer::make(int nstrm,
      ...
      return gnuradio::make_block_sptr<mapper_muxer_impl>(
        nstrm, phase1_modtype, phase2_modtype, phase3_modtype,
        n_ofdm_syms, sd_num, use_polar,
                        ...
                        if (pmt::is_number(msg)) {
                          ...
                          if (d_valid_mod_types.count(d_new_phase1_modtype) &&
                            ...
                            if ((d_new_phase2_modtype >= d_new_phase1_modtype) &&
                                (d_new_phase3_modtype >= d_new_phase2_modtype)) {
                              ...
                              throw std::runtime_error(
                                "mapper_muxer: Each phase must have mod order equal or greater than previous phase"
    ...
    void mapper_muxer_impl::forecast(int noutput_items,
      ...
      if (!d_deterministic_input) {
        ...
        needed_items = d_nstrm * d_n_ofdm_syms * d_sd_num * d_phase1_modtype
                       - d_nstrm * 64 * d_phase1_modtype;
        if (d_code_rate > 0) {
          double R = code_rates[d_code_rate];
          needed_items = int(std::floor(needed_items * R));
          needed_items = needed_items + (d_phase1_modtype - (needed_items % d_phase1_modtype));
        }
    ...
    int mapper_muxer_impl::general_work(int noutput_items,
      ...
      int frame_data_bits_out = frame_data_syms * d_phase1_modtype;
      ...
      if (d_code_rate > 0) {
        ...
        in_bits_needed = in_bits_needed + (d_phase1_modtype - (in_bits_needed % d_phase1_modtype));
        if (d_debug) {
          std::cout << "in_bits_needed=" << in_bits_needed
              << "in_bits_needed%modtype=" << in_bits_needed % d_phase1_modtype << std::endl;
        }
      }
